...
\end{itemize}
\paragraph{Condition}
We need to impose that the assignment $Z$ is independent of the potential
outcome $Y(z,d)$ and the treatment $D$ (given $X$). From now on we omitted the
conditioning on $X$ for simplicity. This is the \emph{exclusion restriction}
assumption. It is similar to $\E\bra{\epsilon\mid X,Z}=0$ assumption in
